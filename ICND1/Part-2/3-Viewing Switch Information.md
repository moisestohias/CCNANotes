...
= IOS Version ===============================
> show version : display informations about the platform series, IOS version, the uptime, the reason of the last reboot, the location of the currently loaded IOS image, RAM size, the installed interfaces, NVRAM size, the exact model number...
...
!Note: duplex mismatch is one of the most common problem with switches, because the speed duplex is negotiated between devices, sometimes it can fail, and on of the device ends up choosing half-duplex, when this happen some of the data will make it through, so you get slow performance and packet loss.
