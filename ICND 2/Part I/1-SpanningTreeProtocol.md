...
+  All devices in a VLAN can send frames to all other devices. In other words, STP/RSTP does not block too many ports, cutting off some parts of the LAN from other parts.
...
>Note: The cost is the sum of the costs of all the switch ports THE FRAME WOULD EXIT IF IT FLOWED OVER THAT PATH. So every switch when it recieves a BPUD from one of its port it adds the cost of that port to the received cost (The sending switch doesn't add the outgoing port cost), which means all BPDUs start with cost of 0 as they exit root bridge.
...
# Bridge-ID (BID): 
A number that allows switches to elect the root a bridge on a network segment, the device with the lowest root bridge, will be the root bridge. 
The Bridge-ID (8 Bytes) value is a combination of two fields, The priority field (2 Bytes) which has a default value of 32768, and can be changed, System-ID (6 Bytes) which is the base MAC address of the switch, that is showen in output of "show version" but in emulated environment (IOU/IOSV) it's the BIA of the first physical interface. 
>Note: Because Cisco supports PVST/Rapid-PVST it needed a mechanism to include the VLAN-ID in the BID, so it stole 12 Bits (enough to represent all extended range VLAN-IDs 0-4096) from the 2 bytes of the priority field and named them System-ID-Extension (aka Extended System-ID) as an extension the System-ID (MAC address field), that because this field can't be configured, it is only defined by the VLAN-ID.
...
# STP Convergence Time: Assuming The STP Happy Triangle Topology: Figure 9-7 Official CerGuide
...
[From CISC community](community.cisco.com/t5/switching/spanning-tree-portfast-default/td-p/1937659)
